...
__attribute__((const))
#endif
static inline _Tp __testlib_abs(const _Tp &x) {
    return x > 0 ? x : -x;
}
...
__attribute__((const))
#endif
static inline _Tp __testlib_min(const _Tp &a, const _Tp &b) {
    return a < b ? a : b;
}
...
__attribute__((const))
#endif
static inline _Tp __testlib_max(const _Tp &a, const _Tp &b) {
    return a > b ? a : b;
}
...
__attribute__((const))
#endif
static inline _Tp __testlib_crop(_Tp value, _Tp a, _Tp b) {
...
__attribute__((const))
#endif
static std::string vtos(const _Tp &t, std::true_type) {
    if (t == 0)
        return "0";
    else {
        _Tp n(t);
...
template<typename _Tp>
static std::string vtos(const _Tp &t, std::false_type) {
    std::string s;
...
template<typename _Tp>
static std::string vtos(const _Tp &t) {
    return vtos(t, std::is_integral<_Tp>());
}

/* signed case. */
template<typename _Tp>
static std::string toHumanReadableString(const _Tp &n, std::false_type) {
    if (n == 0)
        return vtos(n);
...
/* unsigned case. */
template<typename _Tp>
static std::string toHumanReadableString(const _Tp &n, std::true_type) {
    if (n == 0)
        return vtos(n);
    int trailingZeroCount = 0;
    _Tp n_ = n;
...
template<typename _Tp>
static std::string toHumanReadableString(const _Tp &n) {
    return toHumanReadableString(n, std::is_unsigned<_Tp>());
}
#else
template<typename T>
...
template<typename _Tp>
static std::string toString(const _Tp &t) {
    return vtos(t);
}
...
public:
    ...
    std::vector<E> perm(_Tp size, E first) {
        ...
        std::vector<E> p(size);
        E current = first;
        for (_Tp i = 0; i < size; i++)
            p[i] = current++;
        if (size > 1)
            for (_Tp i = 1; i < size; i++)
    ...
    std::vector<_Tp> distinct(int size, _Tp from, _Tp to) {
        ...
        if (expected < double(n)) {
            ...
            std::vector<_Tp> p(perm(int(n), from));
    ...
    std::vector<_Tp> distinct(int size, _Tp upper) {
        ...
        return distinct(size, _Tp(0), upper - 1);
    }
    ...
    std::vector<_Tp> partition(int size, _Tp sum, _Tp min_part) {
        ...
        std::vector<_Tp> septums(size);
        std::vector<_Tp> d = distinct(size - 1, _Tp(1), _Tp(sum + size - 1));
    ...
    std::vector<_Tp> partition(int size, _Tp sum) {
        return partition(size, sum, _Tp(1));
    }
};
...
__attribute__ ((format (printf, 4, 5)))
#endif
NORETURN void expectedButFound(TResult result, _Tp expected, _Tp found, const char *prependFormat = "", ...) {
...
template<class _Tp>
struct __testlib_enable_if<true, _Tp> {
    typedef _Tp type;
};

template<typename _Tp>
typename __testlib_enable_if<!is_iterable<_Tp>::value, void>::type __testlib_print_one(const _Tp &t) {
    std::cout << t;
}
...
typename __testlib_enable_if<is_iterable<_Tp>::value, void>::type __testlib_print_one(const _Tp &t) {
    bool first = true;
    for (typename _Tp::const_iterator i = t.begin(); i != t.end(); i++) {
        if (first)
            first = false;
        else
            std::cout << " ";
...
template<class _Tp>
struct is_iterator<_Tp, typename __testlib_enable_if<std::is_array<_Tp>::value>::type> {
...
template<typename _Tp>
_Tp optValueToIntegral(const std::string &s, bool nonnegative);
...
template<typename _Tp>
_Tp optValueToIntegral(const std::string &s_, bool nonnegative) {
...
template<typename _Tp>
_Tp opt(std::false_type is_floating_point, int index);
...
template<typename _Tp>
_Tp opt(std::true_type /*is_floating_point*/, int index) {
    return _Tp(optValueToLongDouble(__testlib_indexToArgv(index)));
}

template<typename _Tp, typename U>
_Tp opt(std::false_type /*is_integral*/, U /*is_unsigned*/, int index) {
    return opt<_Tp>(std::is_floating_point<_Tp>(), index);
}

template<typename _Tp>
_Tp opt(std::true_type /*is_integral*/, std::false_type /*is_unsigned*/, int index) {
    return optValueToIntegral<_Tp>(__testlib_indexToArgv(index), false);
}

template<typename _Tp>
_Tp opt(std::true_type /*is_integral*/, std::true_type /*is_unsigned*/, int index) {
    return optValueToIntegral<_Tp>(__testlib_indexToArgv(index), true);
}
...
template<typename _Tp>
_Tp opt(int index) {
    return opt<_Tp>(std::is_integral<_Tp>(), std::is_unsigned<_Tp>(), index);
}
 ...
 */
template<typename _Tp>
_Tp opt(int index, const _Tp &default_value) {
    if (index >= int(__testlib_argv.size())) {
        return default_value;
    }
    return opt<_Tp>(index);
}
...
template<typename _Tp>
_Tp opt(std::false_type is_floating_point, const std::string &key);
...
template<typename _Tp>
_Tp opt(std::true_type /*is_integral*/, const std::string &key) {
    return _Tp(optValueToLongDouble(__testlib_keyToOpts(key)));
}

template<typename _Tp, typename U>
_Tp opt(std::false_type /*is_integral*/, U, const std::string &key) {
    return opt<_Tp>(std::is_floating_point<_Tp>(), key);
}

template<typename _Tp>
_Tp opt(std::true_type /*is_integral*/, std::false_type /*is_unsigned*/, const std::string &key) {
    return optValueToIntegral<_Tp>(__testlib_keyToOpts(key), false);
}

template<typename _Tp>
_Tp opt(std::true_type /*is_integral*/, std::true_type /*is_unsigned*/, const std::string &key) {
    return optValueToIntegral<_Tp>(__testlib_keyToOpts(key), true);
}
 ...
 */
template<typename _Tp>
_Tp opt(const std::string &key) {
    return opt<_Tp>(std::is_integral<_Tp>(), std::is_unsigned<_Tp>(), key);
}
 ...
 */
template<typename _Tp>
_Tp opt(const std::string &key, const _Tp &default_value) {
    if (!has_opt(key)) {
        return default_value;
    }
    return opt<_Tp>(key);
